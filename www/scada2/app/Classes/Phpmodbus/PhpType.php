...
namespace App\Classes\Phpmodbus;
...
class PhpType
{
    ...
    {
        // Check the data
        if (!is_array($data) || count($data) < 2 || count($data) > 4 || count($data) == 3) {
    ...
    private static function combineBytes($data, $endianness)
    {
        ...
        // Combine bytes
        if ($endianness == 0) {
            $value = (($data[3] & 0xFF) << 16) | (($data[2] & 0xFF) << 24) | (($data[1] & 0xFF)) | (($data[0] & 0xFF) << 8);
        } else {
            $value = (($data[3] & 0xFF) << 24) | (($data[2] & 0xFF) << 16) | (($data[1] & 0xFF) << 8) | (($data[0] & 0xFF));
        }
